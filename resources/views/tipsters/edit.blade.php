        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h1>Editar Tipster:</h1>
                </div>

                <div class="card-body">
                    <form action="{{ route('tipsters.update', $tipster) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="nombre" id="nombre" 
                                   ...
                                   value="{{ old('nombre', $tipster->nombre) }}" required>
                            @error('nombre')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="pais" id="pais" 
                                   ...
                                   value="{{ old('pais', $tipster->pais) }}" required>
                            @error('pais')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="user_id" id="user_id" class="form-control @error('user_id') is-invalid @enderror" required>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('user_id', $tipster->user_id) == $user->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="grupo_id" id="grupo_id" class="form-control @error('grupo_id') is-invalid @enderror" required>
                                @foreach($grupos as $grupo)
                                    <option value="{{ $grupo->id }}" {{ old('grupo_id', $tipster->grupo_id) == $grupo->id ? 'selected' : '' }}>
                                        {{ $grupo->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mt-3">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Actualizar Tipster
                            </button>
